...
// Raw precinct strings carry whitespace/case variants ("Darlinghurst ") —
// compare normalized so deduped precinct tabs still match every gallery.
function samePrecinct(a, b) {
...
export function filterGalleries(galleries, { search, precinct, sort }) {
  ...
  if (precinct !== 'all') {
    nextGalleries = nextGalleries.filter((gallery) => samePrecinct(gallery.precinct, precinct))
  }
...
export function filterExhibitions(
  ...
  if (precinct !== 'all') {
    nextExhibitions = nextExhibitions.filter((exhibition) => {
      const gallery = getGalleryBySlug(galleries, exhibition.gallerySlug)
      return samePrecinct(gallery?.precinct, precinct)
    })
  }
