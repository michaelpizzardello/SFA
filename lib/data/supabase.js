...
export async function loadSupabaseGalleries() {
  const rows = await requestSupabaseJson(
    '/rest/v1/galleries?select=id,slug,name,location,precinct,suburb,postcode,address,latitude,longitude,website,instagram,phone,email,opening_hours,about&order=name.asc'
  )
