import { buildManualGalleries, loadAdminStore, mapAdminRecordToExhibition } from '../admin/store'
...
import { loadSupabaseGalleries } from './supabase'
...
export async function loadSiteData() {
  ...
  const galleries = galleryDirectory.galleries
  const source = galleryDirectory.source

  try {
    const adminStore = await loadAdminStore()

    if (adminStore.exhibitions.length) {
      const adminExhibitions = filterPublicExhibitionsByRecency(
        adminStore.exhibitions.map(mapAdminRecordToExhibition)
      )
      const manualGalleries = buildManualGalleries(adminExhibitions, galleries)

      return {
        galleries: [...galleries, ...manualGalleries],
        exhibitions: adminExhibitions,
        source: source === 'supabase' ? 'supabase-admin-store' : 'admin-store'
      }
    }
  } catch (error) {
    logFallback('Falling back to sheet data because admin store loading failed.', error)
  }
  ...
  return {
    galleries,
    exhibitions: filterPublicExhibitionsByRecency(sheetData.exhibitions),
    source
  }
}
