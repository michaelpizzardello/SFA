...
export async function loadSiteData() {
  ...
  try {
    const publicRecords = await loadSupabasePublicExhibitions()

    if (publicRecords.length) {
      const adminExhibitions = filterPublicExhibitionsByRecency(
        publicRecords.map(mapAdminRecordToExhibition)
      )
    ...
    logFallback('Falling back to sheet data because Supabase exhibition loading failed.', error)
  }
