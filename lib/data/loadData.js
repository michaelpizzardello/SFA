...
function exhibitionKey(exhibition) {
  return `${exhibition.gallerySlug}::${String(exhibition.title || '').trim().toLowerCase()}`
}

export async function loadSiteData() {
  const galleryDirectory = await loadGalleryDirectoryData()
  let galleries = galleryDirectory.galleries

  // Dashboard-managed (Supabase) published exhibitions — authoritative when present.
  let supabaseExhibitions = []
  try {
    supabaseExhibitions = (await loadSupabasePublicExhibitions()).map(mapAdminRecordToExhibition)
  } catch (error) {
    logFallback('Supabase exhibition loading failed.', error)
  }

  // Google Sheet (legacy bulk catalogue) galleries + exhibitions.
  let sheetData = { galleries: [], exhibitions: [], source: 'empty' }
  try {
    sheetData = await loadSheetSiteData()
  } catch (error) {
    logFallback('Sheet loading failed.', error)
  }
  ...
  // Merge exhibitions: Supabase rows win; add sheet rows that aren't already represented.
  const seen = new Set(supabaseExhibitions.map(exhibitionKey))
  const mergedExhibitions = [...supabaseExhibitions]
  for (const exhibition of sheetData.exhibitions) {
    const key = exhibitionKey(exhibition)
    if (!seen.has(key)) {
      seen.add(key)
      mergedExhibitions.push(exhibition)
    }
  }
