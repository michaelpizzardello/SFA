...
async function loadSiteDataUncached() {
  ...
  const hasConfiguredPublicSource = Boolean(
    process.env.NEXT_PUBLIC_SUPABASE_URL ||
      process.env.NEXT_PUBLIC_GALLERIES_SHEET_URL ||
      process.env.NEXT_PUBLIC_EXHIBITIONS_SHEET_URL
  )
  if (hasConfiguredPublicSource && !galleries.length && !mergedExhibitions.length) {
    throw new Error('Public catalogue sources returned no data.')
  }
