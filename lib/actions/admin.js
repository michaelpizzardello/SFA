...
export async function setGalleryHiddenAction(formData) {
  ...
  if (id) {
    const admin = createAdminSupabase()
    const { error } = await admin.from('galleries').update({ hidden_by_admin: hidden }).eq('id', id)
...
export async function setExhibitionHiddenAction(formData) {
  ...
  if (id) {
    const admin = createAdminSupabase()
    const { error } = await admin.from('admin_exhibitions').update({ hidden_by_admin: hidden }).eq('id', id)
    if (error) throw new Error(error.message)
    updateTag(PUBLIC_SITE_DATA_CACHE_TAG)
  }
