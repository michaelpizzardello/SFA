...
function mapExhibitionRow(row, index, galleriesBySlug) {
  ...
  return {
    id: readFirstFilled(row, ['id'], `ex-sheet-${index + 1}`),
    gallerySlug,
    title,
...
export async function loadSiteData() {
  const galleryCsvUrl = import.meta.env.VITE_GALLERIES_SHEET_URL
  const exhibitionCsvUrl = import.meta.env.VITE_EXHIBITIONS_SHEET_URL
